...
IS_FISH = False         # True to turn boids into fish, False for birds
...
class Boid(pg.sprite.Sprite):
    ...
    def __init__(self, grid, drawSurf, isFish=False):
        ...
        self.color.hsva = (random.randint(0,360), 90, 90) #if cHSV is None else cHSV # randint(5,55) #4goldfish
        ...
        self.rect = self.image.get_rect(center=(random.randint(50, maxW - 50), random.randint(50, maxH - 50)))
        self.ang = random.randint(0, 360)  # random start angle, & position ^
    ...
    def update(self, dt, speed, ejWrap=False):
        ...
        margin = 42
        self.ang = self.ang + random.randint(-4, 4)
        ...
        if (ncount := len(neiboids)) > 1:
            ...
            for nBoid in neiboids:  # adds up neighbor vectors & angles for averaging
                ...
                yat += math.sin(math.radians(nBoid.ang))
                xat += math.cos(math.radians(nBoid.ang))
            tAvejAng = math.degrees(math.atan2(yat, xat))
...
class BoidGrid():
    """tracks boids in spatial partition grid"""
...
def main():
    ...
    for _ in range(BOIDZ):
        nBoids.add(Boid(boidTracker, screen, IS_FISH))
    ...
    while True:
        ...
        linux_ss_signal.check_for_final_events_and_maybe_die()
    
        # for e in pg.event.get(): # quits if any key or mouse button is pressed
        #     if e.type in linux_ss_signal.EVENTS_TO_DIE_FOR:
        #         pg.quit()
        #         return
        ...
        # if true, displays the fps in the upper left corner, for debugging or bragging
        if SHOWFPS:
            # screen.blit(font.render(str(int(clock.get_fps())), True, [0,200,0]), (8, 8))
            linux_ss_signal.show_fps(screen, clock)
